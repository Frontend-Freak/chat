...
export function createMassage(message: ChatMessage): HTMLElement | null {
	const template: HTMLTemplateElement | null = document.querySelector("#messageTemplate");
	...
	const nameElement = templateContent.querySelector(".user__name");
	if (nameElement) nameElement.textContent = `${message.user.name}: `;
	...
	const timeElement: HTMLElement | null = templateContent.querySelector("#timeMassage");
	if (timeElement) timeElement.textContent = message.updatedAt ? new Date(message.updatedAt).toTimeString().slice(0, 5) : new Date().toTimeString().slice(0, 5);
...
function loadMoreMessages() {
	...
	const nextMessages: ChatMessage[] = allMessages.slice(loadedCount, loadedCount + loadSize);
	...
	nextMessages.reverse().forEach((msg) => {
		...
		if (msgElement) chatWindow.insertBefore(msgElement, chatWindow.firstChild);
	});
...
export async function renderMessageHistory() {
	...
	try {
		...
		const result: { messages: ChatMessage[] } = await response.json();
